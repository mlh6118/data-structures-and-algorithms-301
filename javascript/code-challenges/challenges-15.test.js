...
const screenForNames = (arr) => {
  const results = [];
  let pattern = /[D|M]{1}[r|s]{1}[s]*[.]{1}[ ]{1}[A-Za-z]+/g;
...
const toTitleCase = (arr) => {
  ...
  arr.forEach(string => {
    results.push(string.charAt(0).toUpperCase() + string.substring(1));
  });
  return results;
};
...
------------------------------------------------------------------------------------------------ */
const isSecure = (url) => {
  let pattern = /(https:\/\/.)(www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)/g;
