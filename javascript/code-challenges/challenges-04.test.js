...
// A callback is just another function within a function.  Here it is going to be a parameter.
const updateAnimal = (arr, callback) => {
  const newArray = []; // const used because an array is not going to be changed.
  // animalStr is the parameter taken in during an iteration
  // callback will eventually be replaced by upper() or lower() which is passed by the test.
  arr.forEach(animalStr => newArray.push(callback(animalStr)));
  return newArray; //Cannot return within a forEach, so it has to be a new line.
}
...
const sortBackwards = (arr) => {
  return arr.sort((a, b) => b - a); // a - b (read as: a to b) is ascending.  b - a is descending.
};
...
const alphabetizeBetter = (arr) => {
  return arr.sort((a, b) => {
    // using a ternary.  condition ? return -1 : otherwise return 1
    // a > b ? -1 : 1;
...
const sortPeople = (arr) => {
  // console.log(arr[0].lastName, arr[1].lastName, arr[2].lastName);
